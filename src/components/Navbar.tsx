...
// import { usePathname } from "next/navigation"; // not used
...
export function Navbar({ theme = "light" }: { theme?: Theme }) {
  ...
  const baseLink = "text-sm transition-colors border-b-2 border-transparent pb-0.5";
    ...
    <header className="mt-2 text-inherit">
      <div className="flex h-16 items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 text-inherit">
          {LINKS.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={cn(baseLink, active ? linkActive : linkIdle)}>
                {t(item.key)}
              </Link>
            );
          })}
        ...
        <button
          ...
          className={cn(
            "md:hidden p-2 rounded-lg cursor-pointer",
      ...
      <div
        className={cn(
          ...
          isDark ? "bg-brand-dark/95 border-white/10 text-white" : "bg-white/95 border-zinc-200 text-zinc-900"
        )}
      >
