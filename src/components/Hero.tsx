...
/** Stable default so dependency arrays don't thrash */
...
function TypewriterText({
  ...
  // Pick random start only after mount (avoids SSR mismatch)
  useEffect(() => {
      ...
      <div className="mx-auto grid w-full gap-10 lg:grid-cols-[minmax(0,1fr)_32rem] lg:gap-16">
        ...
        <div className="min-w-0">
          <h1 className="text-brand font-normal leading-[1.1] tracking-tight text-4xl md:text-6xl lg:text-7xl">
            <span className="inline-block md:text-[58px]" style={{ minWidth: `${longestCh}ch` }}>
              <TypewriterText phrases={phrases} typeSpeed={40} holdTime={6000} />
            </span>
            <span className="block text-accent md:text-[58px]">
              {t("role")}
              <span className="text-brand">.</span>
            </span>
          </h1>

          <p className="mt-6 max-w-lg text-2xl leading-8 text-zinc-700">
            {t("description")}
          </p>

          <p className="mt-10 max-w-[60ch] text-sm italic text-zinc-500">
            “{t("quote")}”
          </p>
        </div>
